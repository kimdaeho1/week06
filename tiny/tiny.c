...
int main(int argc, char **argv) {         //입력된 인자의 개수, 명령줄 인자의 문자열 배열. 
  ...
  while (1) {                         //메인 루프. 클라이언트 요청이 있을떄마다 처리
    ...
    connfd = Accept(listenfd, (SA *)&clientaddr, &clientlen);  //클라 요청 수락 : 클라가 연결을 요청하면 accept함수가 이를 수락하고,
...
void doit(int fd) //클라이언트 요청을 처리하는 역할
{
  ...
  if ((strcasecmp(method, "GET")) && (strcasecmp(method, "HEAD")))  //다른 메소드POST를 요청하면, main루틴으로 들어오고
  {
  ...
  if (is_static)  //정적 콘텐츠 처리 : 파일이 정적이면 처리. 
  {
    ...
    serve_static(fd, filename, sbuf.st_size, method); //파일 크기를 기반으로 정적 파일을 클라에게 전송
  }
  ...
  {       //파일이 CGI스크립트와 같은 동적 컨텐츠인 경우 처리
    ...
    serve_dynamic(fd, filename, cgiargs, method); //프로그램 실행 후 결과를 클라에 전송
  }
}
...
void serve_static(int fd, char *filename, int filesize, char *method) //HTTP요청에 대해 정적 파일을 응답으로 제공하는 역할. 주어진 파일을 읽어서 HTTP응답으로 클라에게 전송
{
  ...
  srcp = Mmap(0 ,filesize, PROT_READ, MAP_PRIVATE, srcfd, 0); //파일 내용을 메모리에 매핑하여 직접 접근한다. Mmap은 파일을 메모리에 매핑하여 파일의 내요을 메모리에서 직접 읽게 한다.
...
void serve_dynamic(int fd, char *filename, char *cgiargs, char *method)
{
  ...
  if (Fork() == 0)
  {
    ...
    setenv("REQUEST_METHOD", method, 1);  //name : 설정할 환경 변수의 이름, value : 환경 변수에 설정할 값. overwrite : 환경 변수를 덮어쓸지 여부를 저장하는 플래그.
                                          //0 : 환경변수가 이미 있으면 덮어쓰지 않는다, 1이면 기존 값을 새로운 값으로 교체한다.
