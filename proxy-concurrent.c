...
int main(int argc, char **argv) { //입력된 인자의 개수, 명령줄 인자의 문자열 배열. 
  ...
  Signal(SIGCHLD, sigchild_handler);  //여기서 호출되면 실행된다.
  ...
  while (1) {                         //메인 루프. 클라이언트 요청이 있을떄마다 처리
    ...
    if (Fork() == 0)
    {
      ...
      Close(clientfd);
      exit(0);
    }  // line:netp:tiny:doit  //요청 처리 : 클라의 요청을 처리하는 함수. 클라이언트가 요청한 URI 분석하고, 정적 또는 동적 컨텐츠를 제공
...
void sigchild_handler(int sig)  //SIGCHLD시그널이 발생하면, 17번(SIGCHID, child process has stopped or terminated)
{
  while (waitpid(-1, 0, WNOHANG)>0);  //wait또는 waitpid로 수가하는게 아니면 자식이 좀비상태임
  return; //-1을 주면, 어떤 자식이든 상관없이 종료된 자식 프로세스를 수거하겠다. 두번째 인자로 0을 주면, 자식의 종료상태를 굳이 받지 않겠다. 
            //WNOHANG은 종료된 자식이 없다면 waitpid가 즉시 반환하도록 한다.
}