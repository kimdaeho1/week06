...
void doit(int clientfd) //클라이언트 요청을 처리하는 역할
{
  ...
  {
    Rio_writen(clientfd, cached_block->data, cached_block->size);
    return;
  }
  ...
  printf("그래서 저장은 뭘로 : %s:%s%s ", url,port, uri);
...
void addcache(cache_block *new_block, int cache_size)
{
  ...
  while(total_size + new_block->size > MAX_CACHE_SIZE)
  {
    removecache();
  }
  printf("total_size = %d\r\n", total_size);
  ...
  cache_head = new_block; //맨앞 캐시가 캐시 헤드가 된다
